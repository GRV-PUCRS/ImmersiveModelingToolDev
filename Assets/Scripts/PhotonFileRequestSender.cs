...
public class PhotonFileRequestSender : MonoBehaviour
{
    ...
    {
        //Logger.Log($"[PROCESS({fileName}] Send File Coroutine ...");
        ...
        //Logger.Log($"[PROCESS({fileName}]    Envia primeira mensagem ...");

        yield return StartCoroutine(WaitForResponse(Events.FILE_TRANSFER_EVENT, message));

        //Logger.Log($"[PROCESS({fileName}]    Primeira mensagem enviada com sucesso");
        ...
        while (messageSizeSend < fileInBytes.Length)
        {
            ...
            //Logger.Log($"[PROCESS({fileName}]  {k-1}  Envia parte da mensagem");
            ...
            if (confirmation)
            {
                //Logger.Log($"[PROCESS({fileName}]        Pedacos enviados corretamente!");
            ...
            {
                //Logger.Log($"[PROCESS({fileName}]        Tentativas de envio de arquivo excedidas!");
            ...
            //Logger.Log($"[PROCESS({fileName}]  messageSize({messageSize}) | messageSizeSend({messageSizeSend}) | nextMessageSize({nextMessageSize}) | FileLenght({fileInBytes.Length})");
        }
        ...
        //Logger.Log($"[PROCESS({fileName}] Arquivo enviado com sucesso!");
    }
    ...
    IEnumerator WaitForResponse(byte eventCode, object[] message)
    {
        ...
        while (!confirmation && timeoutCounter <= maxTimemoutCounter)
        {
            ...
            //Logger.Log("Espera timeout ou receber confirmacao");
            yield return new WaitUntil(() => confirmation || isTimeout);

            Logger.Log("Sau do loop");
        }
        ...
        //Logger.Log("Fim timeout " + confirmation + "  " + isTimeout);
    }
