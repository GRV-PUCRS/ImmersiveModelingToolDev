...
public class ObjectStore : Singleton<ObjectStore>
{
    ...
    */
    ...
    {
        Debug.Log("[ObjectStore] Drag Begin: " + obj.name);

        if (obj == baseLibrary) return;
    ...
    private void OnObjectDragEnd(Transform obj)
    {
        ...
        if (toDelete && obj != objectsContainerRenderer.transform)
        {
            ...
            else
            {
                ...
                // Remove objeto de oclusao da lista de objetos
                if (dragUI.GetComponent<OcclusionObject>() != null) OculusManager.Instance.RemoveOcclusionObject(dragUI);
                */

                EventManager.TriggerObjectDeleted(obj.gameObject);
