...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		private bool customCameraRay = false;
		...
		void TransformSelected()
		{
			...
			{
				if (customCameraRay)
					if (Input.mousePosition.x < cameraAnchor.x) return;
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			while(!Input.GetMouseButtonUp(0))
			{
				Ray mouseRay;
				if (customCameraRay)
					mouseRay = myCamera.ScreenPointToRay(new Vector2((Input.mousePosition.x - cameraAnchor.x) * xFactor, Input.mousePosition.y * yFactor));
				else
					mouseRay = myCamera.ScreenPointToRay(Input.mousePosition);
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				Ray ray;
				if (customCameraRay)
					ray = myCamera.ScreenPointToRay(new Vector2((Input.mousePosition.x - cameraAnchor.x) * xFactor, Input.mousePosition.y * yFactor));
				else
				...
				if (Physics.Raycast(ray, out hitInfo, Mathf.Infinity, selectionMask))
				{
					...
					EventManager.TriggerObjectSelected(target);
				}else{
		...
		public void SetCameraArgs(Vector3 cameraAnchor, float xFactor, float yFactor)
		{
			customCameraRay = true;
